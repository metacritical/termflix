...
RESULTS=$(printf '%s' "$episode_list" | fzf \
    ...
    --preview '
        ...
        # Get episode data
        ep_no=$(echo {} | cut -d"|" -f1)
        e_info=$(echo "$SEASON_DETAILS" | jq -c --argjson n "$ep_no" ".episodes[] | select(.episode_number == \$n)")
        ...
        e_name=$(echo "$e_info" | jq -r ".name")
        e_runtime=$(echo "$e_info" | jq -r ".runtime // 45")
        
        # Get series-level data for Genre/Cast
        s_genres=$(echo "$SERIES_METADATA" | jq -r "[.genres[]?.name] | join(\", \") // \"Drama\"" 2>/dev/null)
        [[ -z "$s_genres" || "$s_genres" == "null" ]] && s_genres="Drama"
        
        # --- UI: Title ---
        echo
        printf "%s%s%s%s\n" "$BOLD" "$PURPLE" "$e_name" "$RESET"
        printf "%s━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━%s\n" "$GRAY" "$RESET"
        echo
        
        # --- UI: Metadata Row (Runtime | Year | Rating) ---
        [[ "$e_runtime" != "null" && -n "$e_runtime" ]] && printf "%s min    " "$e_runtime"
        [[ "$e_date" != "TBA" && "$e_date" != "null" ]] && printf "%s    " "${e_date:0:4}"
        if [[ "$e_rating" != "N/A" && "$e_rating" != "0" && "$e_rating" != "null" ]]; then
            printf "%s⭐ %.1f%s" "$YELLOW" "$e_rating" "$RESET"
        fi
        echo
        echo
